...
| 4 | Agent access: MCP endpoint + goal cards | 🚧 endpoint **live for Livewire** 2026-08-25 — Hermes Agent querying `marts` via Cloud Run, raw access verified denied from the agent's seat ([hermes-mcp/](hermes-mcp/), [docs/phase-4-hermes-endpoint.md](docs/phase-4-hermes-endpoint.md)); first three goal cards active with targets (sales won, AR over 90, subscription leak), served by the endpoint as `list_goal_cards` / `get_goal_card` — [goal-cards/](goal-cards/README.md) |
...
goal-cards/  Phase 4: KPI goal cards for the agents, one dir per client
