...
def pending_blobs(bucket, fmt_key):
    """Unprocessed uploads under one format prefix, oldest first."""
    blobs = [b for b in bucket.list_blobs(prefix=f"{fmt_key}/")
             if not b.name.endswith("/") and b.size]
...
def main():
    ...
    if not bucket.exists():
        raise RuntimeError(
            f"drop bucket gs://{bucket_name} does not exist — "
            f"run ./scripts/09-vendor-drop.sh {cfg.slug}")

    bq = bq_mod.client_for(cfg)
    total, files = 0, 0
    for fmt_key in sorted(tabular.FORMATS):
        for blob in pending_blobs(bucket, fmt_key):
